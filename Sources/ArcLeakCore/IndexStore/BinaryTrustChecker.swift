// Part of the macOS-only IndexStore layer (its sole consumer is the
// IndexStoreDB-gated reader), and it touches `stat.st_mode`, whose width differs
// across platforms — so it is gated with the rest of the index code.
#if canImport(IndexStoreDB)
    import Foundation

    /// Defence-in-depth check that a dylib path is safe to hand to
    /// `IndexStoreLibrary.init` (which `dlopen`s it). Refuses any path whose
    /// target is not a regular file, is world- or group-writable, or is owned by
    /// some *other* user.
    ///
    ...
    /// root-only ownership would reject exactly the toolchain the user builds
    /// with. The retained gate — regular file, not world/group-writable, owned by
    /// root OR the current user — still blocks a dylib planted by another account
    /// or in a world-writable directory, which is the realistic local threat.
    ///
