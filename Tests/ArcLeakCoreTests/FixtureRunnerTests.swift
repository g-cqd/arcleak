...
///
/// Expectations are markers in the fixtures themselves:
///   `// arcleak-expect: <rule-id>[, <rule-id>…]` — a finding on this line
///   `// arcleak-expect-suppressed: <rule-id>`    — a suppressed finding on this line
///
...
@Suite struct FixtureRunnerTests {
    ...
    static func expectations(in source: String, marker: String) -> Set<Expectation> {
    ...
    func leakFixtures() throws {
        ...
        for url in try Self.fixtureURLs(in: "Leaks") {
            ...
            let expected = Self.expectations(in: source, marker: "arcleak-expect: ")
    ...
    func cleanFixtures() throws {
        ...
        for url in try Self.fixtureURLs(in: "Clean") {
            ...
            let expectedSuppressed = Self.expectations(in: source, marker: "arcleak-expect-suppressed: ")
